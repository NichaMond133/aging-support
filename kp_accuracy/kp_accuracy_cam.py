...
# Ideal keypoints json file
json_keypoints = "ideal_kp/avg_kp_diff_chin_tuck.json"
...
def load_average_differences(json_file):
    """
...
def calculate_accuracy(actual_kp, adjusted_kp):
    ...
    max_distance = 100  # Threshold for maximum reasonable deviation
...
def main():
    ...
    try:
        ...
        if not cap.isOpened():
            logging.error(f"Error opening camera")
            return
        ...
        # Load the average differences from the specified JSON file
        avg_differences = load_average_differences(json_keypoints)
        ...
        while cap.isOpened():
            ...
            if results and results[0].keypoints.xy is not None:
                ...
                if keypoints.shape[0] > 0:
                    if time.time() - start_time <= adjust_time:
                        ...
                        # Calculate accuracy
                        accuracy = calculate_accuracy(keypoints, adjusted_kp)
                        cv2.putText(
                            frame,
